# prepare-bundle.ps1 — 预下载 Android SDK 工具链及系统镜像到 src-tauri\sdk-bundle\
# Windows 版。打包进安装包后，客户端首次运行从 bundle 部署，零网络依赖开箱即用。
# 用法：在项目根目录执行  powershell -ExecutionPolicy Bypass -File scripts\prepare-bundle.ps1
# 可挂代理：$env:HTTPS_PROXY="http://127.0.0.1:7897"
...
Write-Host "==> 架构: $Arch  JRE arch: $JreArch  镜像: $SysImage"
...
# ---- 1. cmdline-tools ----
Write-Host "==> [1/5] 下载 cmdline-tools..."
...
# ---- 2. JRE ----
Write-Host "==> [2/5] 下载 JRE (Temurin 17 $JreArch)..."
...
# ---- 3. platform-tools ----
Write-Host "==> [3/5] 下载 platform-tools..."
...
Write-Host ""
Write-Host "========================================"
Write-Host "  Bundle 准备完成 (全离线/开箱即用)"
Write-Host "========================================"
$Size = (Get-ChildItem $BundleDir -Recurse | Measure-Object -Property Length -Sum).Sum / 1MB
Write-Host ("  大小: {0:N0} MB" -f $Size)
Write-Host ""
Write-Host "下一步: cargo tauri build"
